...
Flight::route('GET /komentar/@id', function($id){
	header ("Content-Type: application/json; charset=utf-8");
	$db = Flight::db();
	$db->select("komentari","*",'idKomentara='.$id);
	$niz=array();
	while ($red=$db->getResult()->fetch_object()){
		$niz[] = $red;
	}
...
Flight::route('DELETE /komentar/@id', function($id){
	header ("Content-Type: application/json; charset=utf-8");
	$db = Flight::db();
	if(($db->select("komentari","*",'idKomentara='.$id))){
		$odgovor["poruka"] = "U bazi ne postoji komentar sa datim ID-em";
		$json_odgovor = json_encode ($odgovor,JSON_UNESCAPED_UNICODE);
		echo $json_odgovor;
		return false;
	}
	if($db->delete("komentari","idKomentara",$id)){
		$odgovor["poruka"] = "Komentar je uspesno obrisan";
		$json_odgovor = json_encode ($odgovor,JSON_UNESCAPED_UNICODE);
		echo $json_odgovor;
		return false;
	}
	else{
		$odgovor["poruka"] = "Doslo je do greske pri brisanju komenata";
		$json_odgovor = json_encode ($odgovor,JSON_UNESCAPED_UNICODE);
		echo $json_odgovor;
		return false;
	}
